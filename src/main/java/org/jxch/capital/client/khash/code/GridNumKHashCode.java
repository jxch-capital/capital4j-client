...
@Slf4j
@Builder
public class GridNumKHashCode implements KHashCode {
    @Builder.Default
    ...
    public List<String> fullHashCodeArr(@NotNull List<KLine> kLines) {
        Double open = priceGetter.apply(kLines.getFirst());
    ...
    public static List<String> hashFull(int skip, @NotNull List<String> hashcodeArr) {
        return skip == 1 ? hashcodeArr : IntStream.range(0, hashcodeArr.size()).filter(i -> i % skip == 0)
                .mapToObj(hashcodeArr::get).toList();
    }
    ...
    public Integer hashLength(int dailyKNum, int skip) {
        return IntStream.range(0, dailyKNum).filter(i -> i % skip == 0).toArray().length;
    }
